...
// ── specific service attendance sheet ─────────────────────────────────────

const generateServiceSheet = async (serviceName, attendanceData, sortedDates) => {
  const children = [];
  ...
  children.push(centered([bold("SERVICE ATTENDANCE SHEET", 26)], { after: 200 }));
  children.push(centered([bold(serviceName.toUpperCase(), 28, "1F3864")], { after: 80 }));

  // Date(s) — could be a recurring service with multiple dates
  if (sortedDates.length === 1) {
    children.push(centered(
      [new TextRun({ text: fmtDate(sortedDates[0]), size: 24 })],
      { after: 300 }
    ));
  } else {
    const first = fmtDate(sortedDates[0]);
    const last  = fmtDate(sortedDates[sortedDates.length - 1]);
    children.push(centered(
      [new TextRun({ text: `${first} – ${last}`, size: 24 })],
      { after: 300 }
    ));
  }

  // For each date (usually just one, but support multiple)
  sortedDates.forEach((date) => {
    const { attendees } = attendanceData[date];
    const total = getDateTotal(attendanceData[date]);

    // Sub-header if multiple dates
    if (sortedDates.length > 1) {
      children.push(heading(fmtDate(date)));
    }

    // Total count badge line
    children.push(new Paragraph({
      children: [
        new TextRun({ text: "Total Attendance: ", bold: true, size: 24 }),
        new TextRun({ text: String(total), bold: true, size: 28, color: "1F3864" }),
      ],
      spacing: { after: 300 },
    }));

    // All attendees as a numbered list, grouped by category
    const allCategories = Object.keys(attendees).sort();
    let counter = 1;

    allCategories.forEach((cat) => {
      const names = attendees[cat];
      if (!names || names.length === 0) return;

      // Category label
      children.push(new Paragraph({
        children: [new TextRun({ text: cat, bold: true, size: 22, color: "1F3864" })],
        spacing: { before: 200, after: 80 },
        border: { bottom: { style: BorderStyle.SINGLE, size: 3, color: "DDDDDD" } },
      }));

      // Names table — 2 columns for readability
      const rows = [];
        ...
        rows.push(new TableRow({
          children: [
            new TableCell({
              children: [new Paragraph({
                children: [
                  new TextRun({ text: `${counter}.  `, size: 20, color: "888888" }),
                  new TextRun({ text: names[i], size: 20 }),
                ],
              })],
            ...
            new TableCell({
              children: [new Paragraph({
                children: names[i + 1] ? [
                  new TextRun({ text: `${counter + 1}.  `, size: 20, color: "888888" }),
                  new TextRun({ text: names[i + 1], size: 20 }),
                ] : [],
              })],
              width: { size: 50, type: WidthType.PERCENTAGE },
              borders: {
  ...
  saveAs(blob, `${serviceName.replace(/\s+/g, "_")}_Attendance.docx`);
};
...
const generateReport = async (serviceName, format = "word", startDate = null, endDate = null) => {
  ...
  // ── specific service → simple attendance sheet ─────────────────────────

  if (serviceName) {
    return generateServiceSheet(serviceName, attendanceData, sortedDates);
  }
